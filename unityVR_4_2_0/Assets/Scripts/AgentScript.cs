...
public partial class AgentScript : MonoBehaviour
{
	...
	/// </summary>
	private bool SendEyePosition = false;
	...
	/// Timout in seconds
	/// </summary>
	protected float SyncModeFrameTimeOut = 5f;
	...
	/// </summary>
	protected float GoodWalkAnimationSpeed = 0.57f;
	...
	float saccTime = 0; // Runtime of current saccade
	float saccTimeDone = 0; // Time done of current saccade
	bool saccadeActive = false; // Boolflag for active saccade
	float m0 = 7f;
	float vpkL = 0, vpkR = 0;
	float RL = 0, RR = 0, A = 0;
	float vL = 0, vR = 0;
	float EccL = 0, EccR = 0;
	Vector3 curDirL,curDirR, tarDirL, tarDirR; // Current direction and target direction of left and right camera
	...
	*/
	protected string[] getGraspAnimationsBeeingExecuted()
	{
		...
		{		
			if( gameObject.animation.IsPlaying( name ) )
			{
	...
	{		
		...
		FovHorizontal = config.FovHorizontal;
		FovVertical = config.FovVertical;
		...
		this.ImageResolutionHeight = 308;
		cameraDisplayHeight = Mathf.RoundToInt( cameraDisplayWidth * Mathf.Tan( Mathf.Deg2Rad * FovVertical / 2 ) / Mathf.Tan( Mathf.Deg2Rad * FovHorizontal / 2 ) );
		//this.cameraDisplayHeight = config.CameraDisplayHeight;
...
//		Debug.Log("ImageResolutionWidth = " + ImageResolutionWidth); TODEL
//		Debug.Log("ImageResolutionHeight = " + ImageResolutionHeight);
		...
		rt_R.useMipMap = true;
		rt_L.useMipMap = true;		
		cameraRight2.camera.targetTexture = rt_R;
		cameraLeft2.camera.targetTexture = rt_L;
	...
	void startSaccade(Vector3 Target)
	{
		...
		curDirL = Vector3.Normalize(LookAtTarget-cameraLeft1.transform.position);
		tarDirL = Vector3.Normalize(newLookAtTarget-cameraLeft1.transform.position);
		curDirR = Vector3.Normalize(LookAtTarget-cameraRight1.transform.position);
		tarDirR = Vector3.Normalize(newLookAtTarget-cameraRight1.transform.position);
		RL = Vector3.Angle(curDirL,tarDirL);
		RR = Vector3.Angle(curDirR,tarDirR);
	...
	public virtual void MoveEyes (float deltaPanLeft, float deltaPanRight, float deltaTilt)
	{
		...
		startSaccade(headLookScript.target);
	}
...
/*
	...
	protected virtual void EyesDirectionReset ()
	{
		...
		// Reset the StandardLookAtPointObject and therefore the headdirection
	...
	{		
		...
		if(animation["walk"] != null)
			animation["walk"].speed =  GoodWalkAnimationSpeed;
	...
	{	
		...
		else if (function == 3) // Van opstal
		{
			A = 1/(1-Mathf.Exp(-RL/m0));
			EccL = m0*Mathf.Log(A*Mathf.Exp(vpkL *(saccTimeDone/saccTime)/m0)/(1+A*Mathf.Exp((vpkL*(saccTimeDone/saccTime)-RL)/m0)));
			A = 1/(1-Mathf.Exp(-RR/m0));
			EccR = m0*Mathf.Log(A*Mathf.Exp(vpkR *(saccTimeDone/saccTime)/m0)/(1+A*Mathf.Exp((vpkR*(saccTimeDone/saccTime)-RR)/m0)));
			vL = vpkL*(1-Mathf.Exp((EccL-RL)/m0));
			vR = vpkR*(1-Mathf.Exp((EccR-RR)/m0));
			cameraLeft1.transform.LookAt(LookAtTarget + (EccL/RL) * (newLookAtTarget - LookAtTarget));
			cameraLeft2.transform.LookAt(LookAtTarget + (EccL/RL) * (newLookAtTarget - LookAtTarget));
			cameraRight1.transform.LookAt(LookAtTarget + (EccR/RR) * (newLookAtTarget - LookAtTarget));
			cameraRight2.transform.LookAt(LookAtTarget + (EccR/RR) * (newLookAtTarget - LookAtTarget));
	...
	protected virtual void Update ()
	{
		...
		case FrameState.FirstFrame:
			...
			{
				cameraLeft1.pixelRect = new Rect(cameraLeft1.GetScreenWidth() - 2.2f *cameraDisplayWidth, cameraLeft1.GetScreenHeight() - 1.1f * cameraDisplayHeight, 
				cameraDisplayWidth, cameraDisplayHeight);
				cameraRight1.pixelRect = new Rect(cameraRight1.GetScreenWidth() - 1.1f *cameraDisplayWidth, cameraRight1.GetScreenHeight() - 1.1f * cameraDisplayHeight, 
				cameraDisplayWidth, cameraDisplayHeight);
			}
			else if (camViewMode == 1) // Option 2: put cameras overlapping at upper right corner, first left then right camera
			{
				cameraLeft1.pixelRect = new Rect(cameraLeft1.GetScreenWidth() - 1.1f *cameraDisplayWidth, cameraLeft1.GetScreenHeight() - 1.1f * cameraDisplayHeight, 
				cameraDisplayWidth, cameraDisplayHeight);
				cameraRight1.pixelRect = new Rect(cameraRight1.GetScreenWidth() - 1.0f *cameraDisplayWidth, cameraRight1.GetScreenHeight() - 1.0f * cameraDisplayHeight, 
				cameraDisplayWidth, cameraDisplayHeight);
			}
			else if (camViewMode == 2) // Option 3: put cameras underneath each other (rat agent)
			{
				cameraLeft1.pixelRect = new Rect(cameraLeft1.GetScreenWidth() - 1.0f *cameraDisplayWidth, cameraLeft1.GetScreenHeight() - 2.0f * cameraDisplayHeight, 
				cameraDisplayWidth, cameraDisplayHeight);
				cameraRight1.pixelRect = new Rect(cameraRight1.GetScreenWidth() - 1.0f *cameraDisplayWidth, cameraRight1.GetScreenHeight() - 1.0f * cameraDisplayHeight, 
				cameraDisplayWidth, cameraDisplayHeight);
			}
			...
			if (this.TimeSinceLastImageWasSend >= this.SimulationTimePerFrame) {	
				...
				RenderTexture.active = cameraRight2.camera.targetTexture;
				cameraRight2.camera.Render ();
				RenderTexture.active = cameraLeft2.camera.targetTexture;
				cameraLeft2.camera.Render ();	
			...
			if (sendingImages && (MySimpleNet.IsConnected)) {
				...
				RenderTexture.active = cameraRight2.camera.targetTexture;
				tex_R.ReadPixels (cameraRight2.camera.pixelRect, 0, 0);
				RenderTexture.active = cameraLeft2.camera.targetTexture;
				tex_L.ReadPixels (cameraLeft2.camera.pixelRect, 0, 0);
		...
		case FrameState.SecondFrame:
			...
			if (MySimpleNet.IsConnected) {			
				...
				{
					// In LookAtMode we allways redirect the cameras to the point of interest
				...
				{
					// Always redirect the cameras relativ to the Agentrotation (not head)
	...
	protected virtual void ProcessMsgGraspRelease( MsgAgentGraspRelease msg )
	{
		...
		if(rightGraspedObject != null)
		{
			...
			Collider col = rightGraspedObject.collider;
			if(col != null)
				col.enabled = true;
