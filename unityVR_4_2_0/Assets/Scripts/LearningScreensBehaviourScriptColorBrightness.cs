...
public class LearningScreensBehaviourScriptWBrightness : BehaviourScript {
    ...
    protected override void AgentInitalization() {	// STILL NEEDED??
    ...
    protected override void ProcessMsgTrialReset(MsgTrialReset msg) {
		...
		brightness = MsgCpy / 10000;
		MsgCpy -= brightness * 10000;
		...
		agentScripts[0].DefaultAgentPosition = new Vector3(2.066f, 0.2485f, -9.0f - (1f * distance));	//-7.06f
		agentScripts[0].DefaultRotation = 0;
	...
	protected void learningScreensTable(int msgcase) {
		...
		switch(color){
			...
			case 1:
				//blue RGB 0 175 255
				color1 = 0.0f; color2 = 175.0f; color3 =  255.0f;
				break;
			case 2:
				//orange RGB 255 135 0
				color1 = 255.0f; color2 = 135.0f; color3 = 0.0f;
				break;
			case 3:
				//green RGB 0 175 0
				color1 = 0.0f; color2 = 175.0f; color3 = 0.0f;
				break;
			case 4:
				//red RGB 175 0 0
				color1 = 175.0f; color2 = 0.0f; color3 = 0.0f;
				break;
			case 5:
				//yellow RGB 255 255 0
				color1 = 255.0f; color2 = 255.0f; color3 = 0.0f;
				break;
		}
		
		switch(brightness){
			case 0:
				//Standard HSV 32 78 255 RGB 255 219 177
				break;
			case 1:
				//HSV 32 78 204 RGB 204 175 204
				color1 *= 0.8f; color2 *= 0.8f; color3 *= 0.8f;
				break;
			case 2:
				//HSV 32 78 153 RGB 153 131 153
				color1 *= 0.6f; color2 *= 0.6f; color3 *= 0.6f;
				break;
			case 3:
				//HSV 32 78 102 RGB 102 87 102
				color1 *= 0.4f; color2 *= 0.4f; color3 *= 0.4f;
				break;
			case 4:
				//HSV 32 78 51 RGB 51 44 51
				color1 *= 0.2f; color2 *= 0.2f; color3 *= 0.2f;
				break;
		}
				
		GameObject.Find("Cube2").renderer.material.color = new Color(color1/255.0f, color2/255.0f, color3/255.0f);
